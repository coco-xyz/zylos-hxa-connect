...
- **Org-level `threadMode` removed**: `getThreadMode()` no longer falls back to org-level `threadMode`. All threads must have explicit `mode` field (set by migration or admin CLI).
